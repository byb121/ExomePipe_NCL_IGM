...
#/***********************************
#note: 12.JUN.2014 script start to produce duplcates-free alignment for freeBayes
#***********************************/

#/***********************************
#note: BGI is still using Illumina 1.5 encoding, extra care is required on quality score encoding (2014 Jun 13)
#***********************************/
...
do
	...
	#/********************** if inputs fastq are from BGI
	READ_FILE1_15="$INDIR/${SAMPLE_ID}_L${LANE}_1.clean.fq"
        READ_FILE2_15="$INDIR/${SAMPLE_ID}_L${LANE}_2.clean.fq"
	READ_FILE1="$WRKGDIR/${SAMPLE_ID}_L${LANE}_1.clean.fq"
        READ_FILE2="$WRKGDIR/${SAMPLE_ID}_L${LANE}_2.clean.fq"
	seqtk seq -VQ64 ${READ_FILE1_15} > ${READ_FILE1} #Transfer FastQ Illumina 1.5 to Sanger/1.9
	seqtk seq -VQ64 ${READ_FILE2_15} > ${READ_FILE2} #Transfer FastQ Illumina 1.5 to Sanger/1.9
	#**********************/
	...
	DUP_FREE_BAM_FILE_LIST="$DUP_FREE_BAM_FILE_LIST-I ${SORTED_BAM_FILE_NODUPS_LANE} "
...
then
	...
	mv $SORTED_BAM_FILE_NODUPS_LANE $DUP_FREE_BAM
else # merge with GATK printReads
	MERGED_BAM="$WRKGDIR/${SAMPLE_ID}.bam"
	MERGED_DUP_FREE_BAM="$WRKGDIR/${SAMPLE_ID}_dup_free.bam"
	...
	echo $'\n'"java -Xmx10g -Djava.io.tmpdir=${JAVA_TMP_DIR} -jar $GATKDIR/GenomeAnalysisTK.jar -T PrintReads -nct 4 -R $REF_FILE $DUP_FREE_BAM_FILE_LIST -o $MERGED_DUP_FREE_BAM"
        java -Djava.io.tmpdir=$JAVA_TMP_DIR -Xmx12g -jar $GATKDIR/GenomeAnalysisTK.jar -T PrintReads -nct 4 -R $REF_FILE $DUP_FREE_BAM_FILE_LIST -o $MERGED_DUP_FREE_BAM
        echo "samtools index $MERGED_DUP_FREE_BAM"
	...
	echo $'\n'"["`date`"]: Starting PICARD to remove duplicates at sample level for dup free bam"
        echo "$Picard_nodups INPUT=$MERGED_DUP_FREE_BAM OUTPUT=$DUP_FREE_BAM METRICS_FILE=$PICARD_LOG REMOVE_DUPLICATES=true ASSUME_SORTED=true TMP_DIR=$PICARD_TEMP"
        $Picard_nodups INPUT=$MERGED_DUP_FREE_BAM OUTPUT=$DUP_FREE_BAM METRICS_FILE=$PICARD_LOG REMOVE_DUPLICATES=true ASSUME_SORTED=true TMP_DIR=$PICARD_TEMP
        echo "["`date`"]: Done!"
        echo "rm $MERGED_DUP_FREE_BAM"
        rm $MERGED_DUP_FREE_BAM
...
#check output file existence and size
if [[ -f $FINAL_BAM ]]; then
	sizeee=$(du -k "$FINAL_BAM" | cut -f 1)
	echo "$FINAL_BAM exists size: $sizeee"
else
	echo "$FINAL_BAM error Error!"
fi

if [[ -f $DUP_FREE_BAM ]]; then
        sizeee=$(du -k "$DUP_FREE_BAM" | cut -f 1)
        echo "$DUP_FREE_BAM exists size: $sizeee"
else
        echo "$DUP_FREE_BAM error Error!"
fi

if [[ -f $BED_OUTPUT ]]; then
        sizeee=$(du -k "$BED_OUTPUT" | cut -f 1)
        echo "$BED_OUTPUT exists size: $sizeee"
else
        echo "$BED_OUTPUT error Error!"
fi
