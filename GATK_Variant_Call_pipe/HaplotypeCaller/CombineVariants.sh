...
if [ ! -d $INPUT_FOLDER ]; then
	exit "Input folder: $INPUT_FOLDER does not exist"
fi

INPUT=""
VCFs=($INPUT_FOLDER/*.vcf)

if [ ${#VCFs[@]} -eq 1 ]; then
	exit "No VCF files folder in the folder!"
fi
...
java -Djava.io.tmpdir=$JAVA_TEMP_DIR -Xmx12g -jar $GATKDIR/GenomeAnalysisTK.jar -T CombineVariants -R $REF_FILE -o $OUTPUT_FILE --assumeIdenticalSamples \
$INPUT
